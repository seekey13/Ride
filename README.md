...
- The unlocked-mount list comes from packet `0x0AE`, which the server sends on zone. The list is saved to your settings, so it survives a reload and only a brand new character/settings file needs a zone to fill it in. It refreshes on every zone, so a newly unlocked mount shows up after the next one. `/ride list` shows what it has.
